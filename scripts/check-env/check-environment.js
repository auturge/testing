...
var _logOutput = false;
var _purgeIfStale = false;

try {
  semver = require('semver');
} catch (e) {
  issues.push(
      'Looks like you are missing some npm dependencies. Run: `yarn install`');
}
...
if (require.main === module) {
  ...
  var engines = require(packageFile).engines;
  checkEnvironment(
      {
  ...
  exec('yarn --version', function(yarnErr, yarnStdout) {
    ...
    if (!semver.satisfies(foundNodeVersion, reqs.requiredNodeVersion)) {
      issues.push(
          ...
          foundNodeVersion + ' Expected: ' + reqs.requiredNodeVersion +
    ...
    if (yarnErr) {
      ...
      issues.push(
          ...
          foundYarnVersion + ' Expected: ' + reqs.requiredYarnVersion +
