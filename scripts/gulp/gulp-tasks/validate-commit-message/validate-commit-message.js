...
module.exports = (commitHeader, disallowSquash, nonFixupCommitHeaders) => {
  ...
  const {header, type, scope, isFixup, isSquash} =
      parseCommitHeader(commitHeader);
  ...
  // If it is a fixup commit and `nonFixupCommitHeaders` is not empty, we only
  // care to check whether there is a corresponding non-fixup commit (i.e. a
  // commit whose header is identical to this commit's header after stripping
  // the `fixup! ` prefix).
  ...
  if (header.length > config.maxLength) {
    error(
        `The commit message header is longer than ${
            config.maxLength} characters`,
        commitHeader);
    return false;
  }
  ...
  if (!type) {
    ...
    error(
        `The commit message header does not match the format of '${
            format}' or 'Revert: "${format}"'`,
        commitHeader);
    return false;
  }
...
function error(errorMessage, commitHeader) {
  console.error(
      `INVALID COMMIT MSG: ${commitHeader}\n => ERROR: ${errorMessage}`);
}
